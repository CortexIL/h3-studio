...
test('the Hebrew beta catalogue matches the English one feature for feature', () => {
  ...
  expect(merged.find((f) => f.id === 'sound')?.title).toBe('לתאר את הסאונד')
})
...
test('the Beta page reads in Hebrew when Hebrew is chosen', async () => {
  ...
  expect(screen.getAllByText('לתאר את הסאונד').length).toBeGreaterThan(0)
  expect(screen.getAllByText('מה זה עושה').length).toBeGreaterThan(0)
