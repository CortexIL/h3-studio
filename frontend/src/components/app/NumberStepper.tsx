...
export function NumberStepper({
  ...
  return (
    ...
    >
      <button type="button" className={button} aria-label={`Decrease ${label.toLowerCase()}`} disabled={value <= min} onClick={() => onChange(value - 1)}>
      ...
      />
      <button type="button" className={button} aria-label={`Increase ${label.toLowerCase()}`} disabled={value >= max} onClick={() => onChange(value + 1)}>
